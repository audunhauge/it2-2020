...
const names = `lightpink	 	        lightpink	    #FFB6C1	255	182	193	12695295
...
lightcyan	 	        lightcyan	#E0FFFF	224	255	255	16777184
paleturquoise	        paleturquoise	#AFEEEE	175	238	238	15658671
...
peachpuff	 	        peachpuff	#FFDAB9	255	218	185	12180223
...
lightsalmon	 	        lightsalmon	#FFA07A	255	160	122	8036607
...
whitesmoke	 	        whitesmoke	#F5F5F5	245	245	245	16119285
...
const nameHex = names.split(/\n/).map(e => e.split(/\s+/)).map(ar => [ar[0],ar[2].substr(1)]);
const hexNameInt = names.split(/\n/).map(e => e.split(/\s+/)).map(ar => [ar[2],ar[0],parseInt(ar[2].substr(1),16)]);
// @ts-ignore
// const name2hex = Object.fromEntries(nameHex);
hexNameInt.sort((a,b) => +a[2] - +b[2])
...
function setup() {
    ...
    const sjekkFarge = (e) => {
        if (e.key == 'Enter') {
            // sjekk om fullstendig hex kode- krever 123456 (ignorerer 123 alternativet)
            // @ts-ignore
            const v = inpHex.value;
            if (v.length === 6) {
                const rgb = parseInt(v,16);  // leser inn hexkoden
                let c;
    ...
    //spanNavn.innerHTML = "darkred is  " + colors["darkred"];
    inpHex.addEventListener("keyup", sjekkFarge);
